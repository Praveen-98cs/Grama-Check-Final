...
const Apply = () => {
  ...
  return (
    ...
    <Container id ='apply'>
      <FormWrap>
        ...
        <FormContent>
          <Form onSubmit={handleSubmit}>
            ...
            <FormInput type = 'NIC No' required value={nic} onChange={(e) => setNic(e.target.value)}/>

            <FormLabel htmlFor='for'> Phone Number </FormLabel>
            <FormInput type = 'Phone Number' required value={phone} onChange={(e) => setPhone(e.target.value)}/>

            <FormLabel htmlFor='for'> Address </FormLabel>
            <FormInput type = 'Address' required value={address} onChange={(e) => setAddress(e.target.value)}/>

            {isPending ? ( <FormButton disabled> Submitting...</FormButton>)
      ...
      {isPending ? ( <FormButton disabled> Checking...</FormButton>): ( <FormButton onClick={()=>{setIsClicked(true)}}> Get Certificate</FormButton> )}
      
      {isClicked && <div>{localStorage.getItem('success')? <Certificate name={name} nic={nic} address={address}/> : <div>{localStorage.getItem('msg')=="NIC is not Valid"?<p>Entered NIC is Ivalid. Please Check once again</p>:<div>{localStorage.getItem('msg')=="Police Validation Failed"?<p>Police Validation Failed. You have Criminal Activities</p>:<p>Address Validayion Failed. Please enter a valid address</p>}</div>}</div>}</div>}
